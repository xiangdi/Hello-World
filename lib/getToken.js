...
module.exports = function (opts) {
  var key, client, redisOption, cookieOption, resolveTokenHeader;
  ...
  client = coRedis(redis.createClient(
    redisOption.port,
    redisOption.host,
    redisOption.options
  ));
  ...
  var clientGet = co.wrap(function *(id){
    return yield client.get(id)
  })

  var clientSet = co.wrap(function *(id, data){
    return yield client.set(id, data)
  })

  var clientDel = co.wrap(function *(id){
    return yield client.del(id)
  })
  ...
  return function *(next) {
    ...
    if (sid) {
      debug('sid %s', sid);
      try {
        json = yield clientGet(sid);
        //logger.warn('token.json', json)
      }catch (e) {
      ...
      debug('sid %s', sid);
      try {
        sid = this.header['x-hxbauth-token'];
        //logger.info(sid)
        json = yield clientGet(sid);
        //logger.warn(json)
      }catch (e) {
